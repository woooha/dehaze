...
def darkChannelOfImage(img, windowSize):
	...
	imgMat = np.matrix([img[:,:,0].reshape(1, h*w)[0],img[:,:,1].reshape(1, h*w)[0],img[:,:,2].reshape(1,h*w)[0]])
	minColorOfImg = np.min(imgMat, axis = 0)
	gray = minColorOfImg.reshape(h, w)
...
# 	normlizedImg = np.matrix(normlizedImg).reshape(h, w)
